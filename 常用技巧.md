...
a = np.random.rand(2,2) ## 生成一个2*2的矩阵，其中每个元素都是随机生成的
...
## 从(0,2]所包含的整数中等概率的生成20个数
d = np.random.random_integers(2,size=20)
d = np.random.random_integers(1,high=2,size=20)
...
f = np.uniform(low=0.0, high=1.0, size=10)  ## 默认就是[0,1]之间的均匀分布
